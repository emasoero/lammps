...
MinQuickmaske::MinQuickmaske(LAMMPS *lmp) : Min(lmp) {}

/* ---------------------------------------------------------------------- */

void MinQuickmaske::init()
{
  Min::init();
...
void MinQuickmaske::setup_style()
{
  double **v = atom->v;
...
void MinQuickmaske::reset_vectors()
{
  // atomic dof
...
int MinQuickmaske::iterate(int maxiter)
{
  bigint ntimestep;
...
*/
      ...
      for (int i = 0; i < nlocal; i++) {
          ...
          vdotfi = v[i][0]*f[i][0] + v[i][1]*f[i][1] + v[i][2]*f[i][2];
          ...
          if (vdotfi < 0.0) {
            last_negative = ntimestep;
              ...
              fdotfi = f[i][0]*f[i][0] + f[i][1]*f[i][1] + f[i][2]*f[i][2];
          
              if (fdotfi == 0.0) scalei = 0.0;
              else scalei = vdotfi/fdotfi;
